...
export class WorkerClient {
  ...
  constructor(_jsFile: string) {
    // _jsFile is upstream's webpack-era path to a copied
    // 'storeWorkerEndpoint.js' (built via path.join + Electron's
    // getAppPath, now stubbed — see src/platform/node-shims/path.ts).
    // That pattern doesn't resolve in Vite, so it's ignored — Vite's
    // new URL(..., import.meta.url) worker syntax bundles and resolves
    // the actual worker script correctly instead, both in dev and in the
    // production Capacitor build.
    this.worker = new Worker(
      new URL('./store.worker.endpoint.ts', import.meta.url),
      { type: 'module' }
    );
